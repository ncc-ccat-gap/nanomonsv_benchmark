...
def post_filter_main(args):
    ...
    with open(args.sv_list_file, 'r') as hin, open(args.output_file, 'w') as hout:
        ...
        if not "Is_Filter" in header:
            header.append("Is_Filter")
        print('\t'.join(header), file = hout)

        for F in dreader:
            #if not "Is_Filter" in F:
            #    F["Is_Filter"] = "PASS"
            if F["Is_Filter"] == None:
                F["Is_Filter"] = "PASS"
            if int(F["Supporting_Read_Num_Tumor"]) < args.min_tumor_support_read:
                if F["Is_Filter"] == "PASS": 
                    F["Is_Filter"] = "Too_Low_Supporting_Read_Num_Tumor"
                else:
                    F["Is_Filter"] = F["Is_Filter"] + ';' + "Too_Low_Supporting_Read_Num_Tumor"
